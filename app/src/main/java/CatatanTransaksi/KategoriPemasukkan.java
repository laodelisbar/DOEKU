...
public class KategoriPemasukkan extends SQLiteOpenHelper {
    ...
    public boolean addPemasukkan(String nama) {
        if (nama.isEmpty()) {
            return false; // Data kosong
        }

        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.query(TABLE_NAME, new String[]{COLUMN_NAMA_KATEGORI_PEMASUKKAN}, COLUMN_NAMA_KATEGORI_PEMASUKKAN + "=?", new String[]{nama}, null, null, null);
    ...
    public boolean deleteCategory(String namaKategori) {
        SQLiteDatabase db = this.getWritableDatabase();
        return db.delete(TABLE_NAME, COLUMN_NAMA_KATEGORI_PEMASUKKAN + "=?", new String[]{namaKategori}) > 0;
    }
